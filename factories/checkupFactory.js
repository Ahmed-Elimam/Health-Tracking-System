...
generateCheckupData = async (overrides = {},patientCount,doctorCount,threadCount,specializationCount) => {
    if(!specializationCount) specializationCount = await Specialization.countDocuments();
    ...
    return {
        createdBy: faker.helpers.arrayElement([patientDoc.userId, doctorDoc.userId]),
...
exports.createCheckup = async (overrides = {},patientCount,doctorCount,threadCount,userCount,specializationCount) => {
    const data = await generateCheckupData(overrides,patientCount,doctorCount,threadCount,userCount,specializationCount);
