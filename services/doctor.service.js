...
exports.getDoctors_Patient = async (query, user) => {
    ...
    for (let i = 0; i < treatings.length; i++) {
        if (treatings[i].isActive == false)
            oldDoctors.push(
                doctorResource_Patient(
                    await treatings[i].doctorId.populate('specialization')
                )
            );
        else
            currentDoctors.push(
...
exports.updateDoctor_Doctor = async (doctorId, doctorData, userId) => {
    const doctor = await Doctor.findById(doctorId).populate('userId');
    if (doctor.userId._id == userId)
        return await Doctor.findByIdAndUpdate(doctorId, doctorData, {
            new: true,
        });
    else throw new Error('You are not allowed to access this doctor');
};
