...
exports.createPrescription = catchAsync(async (req, res, next) => {
    ...
    if (user.role === 'doctor') {
        ...
        const isTreating = await PrescriptionService.checkDoctorTreatsPatient(
            doctorId,
            patientId
        );
        if (!isTreating) {
            return next(
                new AppError(
                    'You are not currently assigned to treat this patient',
                    403
                )
            );
        }
...
exports.getPrescriptions = catchAsync(async (req, res, next) => {
    ...
    if (user.role === 'patient') {
        ...
        prescriptions = await PrescriptionService.getAllDoctorPrescriptions(
            user.doctorId,
            user.id
        );
...
exports.getPrescription = catchAsync(async (req, res, next) => {
    ...
    if (
        user.role === 'doctor' &&
        prescription.doctorId.toString() !== user.doctorId.toString()
    ) {
    ...
    if (
        user.role === 'patient' &&
        prescription.patientId.toString() !== user.patientId.toString()
    ) {
        return next(new AppError('Access denied', 403));
    }
...
exports.updatePrescription = catchAsync(async (req, res, next) => {
    ...
    if (user.role === 'patient') {
        if (prescription.createdBy.toString() !== user.id.toString()) {
            return next(
                new AppError(
                    'Access denied - you can only update prescriptions created by you',
                    403
                )
            );
        }
        ...
        const hoursDifference =
            await PrescriptionService.getPrescriptionCreatedTime(prescription);

        if (hoursDifference > 48) {
            return next(
                new AppError(
                    'Cannot update prescription after 48 hours of creation',
                    403
                )
            );
        }
        ...
        return next(
            new AppError('Doctors are not allowed to edit prescriptions', 403)
        );
    } else if (user.role !== 'admin' && user.role !== 'super-admin') {
        return next(new AppError('Unauthorized', 403));
    }
...
exports.deletePrescription = catchAsync(async (req, res, next) => {
    ...
    if (user.role !== 'admin' && user.role !== 'super-admin') {
        return next(
            new AppError('Only administrators can delete prescriptions', 403)
        );
    }
