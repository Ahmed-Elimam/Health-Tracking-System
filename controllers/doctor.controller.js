...
exports.getDoctor = catchAsync( async (req, res,next) => {
    if(req.user.role == "admin" || req.user.role == "super-admin"){
        ...
        try{
            const doctor = await DoctorService.getDoctor_Doctor(req.params.id,req.user.id);
            res.status(200).json(doctor);
        }
        catch(error){
        ...
        const doctor = await DoctorService.getDoctor_Patient(req.params.id,req.user);
        res.status(200).json(doctor);
    }
});
...
exports.updateDoctor = catchAsync( async (req, res,next) => {
    if(req.user.role == "admin" || req.user.role == "super-admin"){
    const doctor = await DoctorService.updateDoctor_Admin(req.params.id, req.body);
    res.status(200).json(doctor);
    }
    else if(req.user.role == "doctor"){
        const doctor = await DoctorService.updateDoctor_Doctor(req.params.id, req.body,req.user.id);
        res.status(200).json(doctor);
    }
    else
        next(new AppError("Not authorized", 403));
