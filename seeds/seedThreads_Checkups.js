...
const patientCount = Patient.countDocuments();
const specializationCount = Specialization.countDocuments();
const DoctorCount = Doctor.countDocuments();
const threadCount = Thread.countDocuments();
async function seed() {
  await dbConnect();
  for (let i = 0; i < 10; i++) {
    const thread = await createThread();
    const randomInt = Math.floor(Math.random() * 3) + 1;
    const randomPatients = Math.floor(Math.random() * Patient.countDocuments());
    const patientDoc = await Patient.findOne().skip(randomPatients).lean();
    const randomSpecialization = Math.floor(Math.random() * Specialization.countDocuments()) + 1;
